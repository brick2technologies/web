...
interface SpaceBackgroundProps {
  ...
  particleColor?: string; // optional override
  ...
  useEffect(() => {
    ...
    const setContrast = () => {
      ...
      if (rgb) {
        ...
        if (lum < 0.5) {
          setResolvedColor("rgba(255,255,255,0.85)"); // dark background → bright particles
        } else {
          setResolvedColor("rgba(0,0,0,0.85)"); // light background → visible dark particles
        }
      } else {
        const media = window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)");
